...
class MongoDataGenerator:
    ...
    def generate_value(field_type: str, specs: Any) -> Any:
        ...
        if "null_probability" in specs and random.random() < specs["null_probability"]:
            return None
        ...
        if field_type == "big_integer":
            ...
            length = random.randint(specs["length_range"][0], specs["length_range"][1])
            ...
            size = random.randint(specs["size_range"][0], specs["size_range"][1])

            if "element_type" in specs:
                return [
                    MongoDataGenerator.generate_value(
                        specs["element_type"], specs["element_specs"]
                    )
                    for _ in range(size)
                ]
    ...
    def insert_records(
        ...
        Args:
            ...
            dress_rehearsal: If True, prints records to stdout instead of inserting
             to MongoDB
        ...
        if dress_rehearsal:
            ...
            for batch in self.generate_records(count):
                ...
                ) / count >= next_percentage_to_report:  # Print progress every 10%
                    print(f"Processed {(total_processed * 100.0) / count}% of records")
                    next_percentage_to_report += 10
            return total_processed

        with MongoClient(mongodb_uri) as client:  # type: MongoClient
